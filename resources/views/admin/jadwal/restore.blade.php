...
<button role="button" class="btn btn-sm btn-info" data-bs-toggle="modal"
    data-bs-target=".formRestore{{ $jadwal->id }}"><i class="fas fa-undo"></i><span class="d-none d-sm-inline">
        {{ __('Restore') }}</span></button>

<!-- Modal -->
<div class="modal fade formRestore{{ $jadwal->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('admin.jadwal.restore', $jadwal->id) }}" method="POST">
                    @method('PUT')
                    @csrf
