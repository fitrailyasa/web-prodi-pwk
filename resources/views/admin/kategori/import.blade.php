    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            @if (auth()->user()->role == 'admin')
                <form method="POST" action="{{ route('admin.kategori.import') }}" enctype="multipart/form-data">
            @endif
            @csrf
            <div class="modal-header">
                <h5 class="modal-title" id="modalFormLabel">{{ __('Upload kategori') }}</h5>
            ...
            <div class="modal-footer">
                <a class="btn btn-success" href="{{ asset('assets/template/kategori-template.xlsx') }}"
                    download="kategori-template.xlsx"><i class="fas fa-download"></i> {{ __('Download Format') }}</a>
