...
<button role="button" class="btn btn-sm btn-info" data-bs-toggle="modal"
    data-bs-target=".formRestore{{ $medpart->id }}"><i class="fas fa-undo"></i><span class="d-none d-sm-inline">
        {{ __('Restore') }}</span></button>

<!-- Modal -->
<div class="modal fade formRestore{{ $medpart->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('admin.medpart.restore', $medpart->id) }}" method="POST">
                    @method('PUT')
                    @csrf
