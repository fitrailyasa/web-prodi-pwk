...
<!-- Modal -->
<div class="modal fade formEdit{{ $staff->id }}" tabindex="-1" role="dialog" aria-hidden="">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                placeholder="name" name="name" id="name" value="{{ old('name', $staff->name) }}"
                                required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input id="image-input" accept="image/*" type="file" id="img-input"
                                ...
                                id="img" value="{{ old('img', $staff->img) }}">
                            @error('img')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('desc') is-invalid @enderror" placeholder="description" name="desc"
                                id="desc" rows="3">{{ old('desc', $staff->desc) }}</textarea>
                            @error('desc')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            @if ($staff->img == null)
                                <img class="img-fluid rounded" width="200px" id="image-preview"
                                    src="{{ asset('assets/profile/default.png') }}" alt="{{ $staff->name }}">
                            @else
                                <img class="img-fluid rounded" width="200px" id="image-preview"
                                    src="{{ asset('assets/img/' . $staff->img) }}" alt="{{ $staff->name }}">
                            @endif
                        </div>
                    </div>
                </div>
