    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            @if (auth()->user()->role == 'admin')
                <form method="POST" action="{{ route('admin.chatbot.import') }}" enctype="multipart/form-data">
            @elseif (auth()->user()->role == 'dosen')
                <form method="POST" action="{{ route('dosen.chatbot.import') }}" enctype="multipart/form-data">
            @endif
            @csrf
            <div class="modal-header">
                <h5 class="modal-title" id="modalFormLabel">{{ __('Upload Chatbot') }}</h5>
            ...
            <div class="modal-footer">
                <a class="btn btn-success" href="{{ asset('assets/template/chatbot-template.xlsx') }}"
                    download="chatbot-template.xlsx"><i class="fas fa-download"></i> {{ __('Download Format') }}</a>
