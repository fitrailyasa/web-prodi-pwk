...
<button role="button" class="btn btn-sm btn-warning mr-2" data-bs-toggle="modal"
    data-bs-target=".formEdit{{ $tentang->id }}"><i class="fas fa-edit"></i><span class="d-none d-sm-inline">
        {{ __('Edit') }}</span></button>

<!-- Modal -->
<div class="modal fade formEdit{{ $tentang->id }}" tabindex="-1" role="dialog" aria-hidden="">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                placeholder="name" name="name" id="name" value="{{ old('name', $tentang->name) }}"
                                required>
                            @error('name')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input id="image-input" accept="image/*" type="file" id="img-input"
                                ...
                                id="img" value="{{ old('img', $tentang->img) }}">
                            @error('img')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('desc') is-invalid @enderror" placeholder="description" name="desc"
                                id="desc" rows="3">{{ old('desc', $tentang->desc) }}</textarea>
                            @error('desc')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            @if ($tentang->img == null)
                                <img class="img-fluid rounded" width="200px" id="image-preview"
                                    src="{{ asset('assets/profile/default.png') }}" alt="{{ $tentang->name }}">
                            @else
                                <img class="img-fluid rounded" width="200px" id="image-preview"
                                    src="{{ asset('assets/img/' . $tentang->img) }}" alt="{{ $tentang->name }}">
                            @endif
                        </div>
                    </div>
                </div>
